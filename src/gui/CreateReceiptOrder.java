...
public class CreateReceiptOrder {
	...
	private boolean selectOrderRoom() {
		//connections
		Connection connection;
		try {
			connection = ConnectionUtils.getMyConnection();
	...
	private boolean showOrderRoom(ArrayList<Order> lsOb) {
		int size = lsOb.size();
	...
	private boolean saveReceiptOrder(int customer_id,int price ,String address, ArrayList<Order> ls) {
		if(customer_id == 0) {
